...
签名文件、密码和发布渠道配置必须通过本地环境变量、CI secret 或未提交的 Gradle 配置注入。当前 Android 模板支持以下四项，缺任何一项都会被 `mf:android-build` 视为不完整配置：
...
这四项可以来自环境变量、CI secret、`apps/<app-name>/android/local.properties`、`apps/<app-name>/android/gradle.properties` 或 `$GRADLE_USER_HOME/gradle.properties`。`android/local.properties` 已加入 Git 忽略规则；keystore 文件建议放在仓库外，或由 CI secret 在构建前解码到临时路径。
...
正式业务 App 不应使用该参数。scaffold 构建写出的 Android build evidence 会标记 `signingMode: "debug-scaffold"`；生产签名配置完整时会标记 `signingMode: "release"`。
