...
- `packages/mobile-bff` now defines the `/api/v1/mobile` typed client, fixture transport, HTTP transport, list filtering/pagination contracts, facets, task logs, second-stage management domains for users/modules/assets/releases/logs, and action receipts; `game-helper-admin-mobile` reads dashboard, device, task, log, user, module, asset, and release data through `src/services/mobile-bff.ts` instead of directly simulating BFF queries inside screens. `src/services/runtime.ts` exports a runtime configuration boundary that can switch from fixture BFF/realtime to HTTP BFF plus WebSocket realtime, with realtime polling fallback reusing the configured mobile BFF client. The management tab shows BFF-backed previews for those second-stage domains while full list/detail/approval pages remain planned.
...
- `game-helper-admin-mobile` still uses local fixture data by default. Runtime configuration entry points now exist for real BFF baseUrl/token/fetch and WebSocket URL/socket factory injection, but actual environment-specific values, server-backed data, and production error handling still need to be wired by the consuming app or bootstrap layer. Full second-stage management pages for users/modules/assets/releases/logs/approvals also remain to be split from the current management preview.
...
3. Continue the `game-helper-admin-mobile` adaptation by providing real environment values to `configureAdminMobileRuntime` or `configureAdminMobileHttpRuntime`, wiring business token retrieval/error handling, and then splitting the current users/modules/assets/releases/logs previews into full second-stage pages.
