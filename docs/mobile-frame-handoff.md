...
- `game-helper-admin-mobile` still uses local fixture data. It needs real WebSocket/BFF transport integration, scanner/clipboard/share wrappers, server-backed pagination/search data flows, and real `/api/v1/mobile` BFF integration.
...
3. Continue the `game-helper-admin-mobile` adaptation by replacing fixture realtime with the real WebSocket/BFF transport, adding scanner/clipboard/share wrappers, server-backed pagination/search data flows, and real `/api/v1/mobile` API clients behind the current fixture-backed screens.
