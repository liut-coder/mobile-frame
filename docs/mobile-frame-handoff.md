...
- `game-helper-admin-mobile` still uses local fixture data by default. It needs the real backend URL/token wiring for `@mobile-frame/mobile-bff`, real WebSocket URL/socket injection for `src/services/realtime.ts`, and server-backed data behind the current typed client contracts. Full second-stage management pages for users/modules/assets/releases/logs/approvals also remain to be split from the current management preview.
...
3. Continue the `game-helper-admin-mobile` adaptation by injecting the real WebSocket URL/socket factory into `src/services/realtime.ts`, switching `src/services/mobile-bff.ts` from fixture data to the HTTP client against the real `/api/v1/mobile` backend, and then splitting the current users/modules/assets/releases/logs previews into full second-stage pages.
