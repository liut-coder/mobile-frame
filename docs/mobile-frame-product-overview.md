...
- Reusable `mobile-bff` package for `/api/v1/mobile` typed clients, fixture-backed page work, HTTP mapping, pagination, filtering, first-stage device/task flows, second-stage users/modules/assets/releases/logs contracts, and admin action receipts.
- Code generators.
...
- Admin-mobile capabilities beyond the first scaffold: providing real environment values to the current HTTP BFF/WebSocket runtime configuration boundary, production server-backed pagination/search data flows, and full second-stage pages for users, modules, assets, releases, logs, and approvals.
...
The current milestone has reached source-level validation, Android debug APK assembly proof, Android Release scaffold proof, Android emulator runtime proof through GitHub Actions run `26906180713`, the first `game-helper-admin-mobile` adaptation pass from `docs/game-helper-admin-mobile-mobile-frame-adaptation.md`, reusable admin-mobile permission gates through `packages/auth-admin`, configurable realtime subscriptions through `packages/realtime`, mobile list filtering/pagination components through `packages/ui-admin`, `/api/v1/mobile` typed client contracts through `packages/mobile-bff`, runtime configuration for HTTP BFF plus WebSocket realtime fallback, second-stage management-domain BFF previews for users/modules/assets/releases/logs, and scanner/clipboard/share/browser action wrappers through the native mock contract. The next closure milestone is production Android signing evidence, native build/runtime proof on iOS, real admin-mobile environment values and server-backed data, and full second-stage management pages.
